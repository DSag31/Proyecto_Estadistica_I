...
datos <- read_parquet("C:/Datos_limpios/datos_filtrados_Desapariciones.parquet") 


##################################Pregunta 1#########################################################
#Análisis del número de afectados entre los años 1985 a 2018 (Desapariciones)
#Gráfica : Histograma.
...
ggplot(df_por_ano, aes(x = yy_hecho, y = n_victimas)) +
  ...
  labs(
    title    = "Número de víctimas de desapariciones por año (1985–2018)",
    x        = "Año",
...
##################################Pregunta 2#########################################################
# MAPA DE CALOR DESAPARICIONES 


# 1. Leer datos de desapariciones
casos_dept <- datos %>%
...
ggplot(map_data) +
  ...
  labs(
    title = "Mapa de calor víctimas de desapariciones por departamento (1985–2018)"
  ) +
  theme_void() +
  theme(
...
ggplot(map_data) +
  ...
  labs(
    title = "Mapa de calor de desapariciones en municipios de Santander\n(1985–2018)"
  ) +
  theme_void() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
ggplot(top_municipios_nombres, aes(x = reorder(MUNICIPIO, -total_), y = total_, fill = DEPARTAMENTO)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Top 5 municipios con más Desapariciones (1985–2018)",
    x = "Municipio",
    y = "Cantidad de desapariciones"
  ) +
  theme_minimal() +
...
# Ver las primeras filas para encontrar la categoría con más 
print(conteo_edades)
...
ggplot(conteo_edades, aes(x = reorder(edad_categoria, -n), y = n)) +
  ...
  labs(title = "Frecuencia de Desapariciones por categoría de edad",
       x = "Rango de Edad",
       y = "Cantidad de Desapariciones") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(conteo_etnias, aes(x = reorder(etnia, -n), y = n, fill = etnia)) +
  ...
  labs(
    title = "Número de desaparicones por grupo étnico (Barras)",
    x = "Grupo Étnico",
...
ggplot(top10_guerrilla, aes(x = reorder(p_str, n), y = n, fill = p_str)) +
  ...
  coord_flip() +
  labs(
    title = "Guerrillas que más afectan a los ciudadanos (Desapariciones)",
    x = "Guerrilla",
    y = "Número de casos"
  ) +
  theme_minimal()
...
ggplot(datos, aes(x = edad_media)) +
  ...
  labs(title = "Distribución de edades de víctimas de desapariciones",
       x = "Edad (estimada)",
       y = "Densidad") +
  theme_minimal()
